...
def solve(question):    
    exp = re.compile('[A-Z]{3}[a-z]{1}[A-Z]{3}')
    cap = re.compile('[a-z]')
    it = re.finditer(exp, question)

    res = []
    for match in it:
        if match.group(0) == "ZAGiLQZ": print "YES"
        ind = match.start(0)
        prev = question[ind-1]
        try:
            next = question[ind+7]
        ...
        if re.match(cap, prev) and re.match(cap, next):
            res.append(match.group(0)[3])
    print ''.join(res)
